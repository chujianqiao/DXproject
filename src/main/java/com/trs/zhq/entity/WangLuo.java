package com.trs.zhq.entity;


public class WangLuo {
    private String SFILENAME;
    private Long NFILELEN;
    private String SCOMFREQSPAN;
    private String SSATELLITE_NAME;
    private String SORBIT_POS;
    private String SPOLARIZATION_TYPE;
    private String STRANSPONDR;
    private Long NCARRIERFREQ;
    private String DDATA_RECBTIME;
    private String DDATA_RECETIME;
    private Long NDATA_RECDURATION;
    private String SMODULATIONMODE;
    private Long NMODULATIONRATE;
    private String SCODETYPE;
    private String SCODERATIO;
    private String SSCRAMBLERTYPE;
    private String SSCRAMBLERRATIO;
    private String SMODFRAMETYPE;
    private String SFRAMERATIO;
    private Long NINFORATE;
    private String SBUSINESSTYPE;
    private String SNETSIGNALTYPE;
    private Long NDEMOD_ERR;
    private Long NDECOD_ERR;
    private String SNETDATA_TYPE;
    private String SIS_COMPRESS_SAVE;
    private String SUSERCOMMENT;

    public String getSFILENAME() {
        return SFILENAME;
    }

    public void setSFILENAME(String SFILENAME) {
        this.SFILENAME = SFILENAME;
    }

    public Long getNFILELEN() {
        return NFILELEN;
    }

    public void setNFILELEN(Long NFILELEN) {
        this.NFILELEN = NFILELEN;
    }

    public String getSCOMFREQSPAN() {
        return SCOMFREQSPAN;
    }

    public void setSCOMFREQSPAN(String SCOMFREQSPAN) {
        this.SCOMFREQSPAN = SCOMFREQSPAN;
    }

    public String getSSATELLITE_NAME() {
        return SSATELLITE_NAME;
    }

    public void setSSATELLITE_NAME(String SSATELLITE_NAME) {
        this.SSATELLITE_NAME = SSATELLITE_NAME;
    }

    public String getSORBIT_POS() {
        return SORBIT_POS;
    }

    public void setSORBIT_POS(String SORBIT_POS) {
        this.SORBIT_POS = SORBIT_POS;
    }

    public String getSPOLARIZATION_TYPE() {
        return SPOLARIZATION_TYPE;
    }

    public void setSPOLARIZATION_TYPE(String SPOLARIZATION_TYPE) {
        this.SPOLARIZATION_TYPE = SPOLARIZATION_TYPE;
    }

    public String getSTRANSPONDR() {
        return STRANSPONDR;
    }

    public void setSTRANSPONDR(String STRANSPONDR) {
        this.STRANSPONDR = STRANSPONDR;
    }

    public Long getNCARRIERFREQ() {
        return NCARRIERFREQ;
    }

    public void setNCARRIERFREQ(Long NCARRIERFREQ) {
        this.NCARRIERFREQ = NCARRIERFREQ;
    }

    public String getDDATA_RECBTIME() {
        return DDATA_RECBTIME;
    }

    public void setDDATA_RECBTIME(String DDATA_RECBTIME) {
        this.DDATA_RECBTIME = DDATA_RECBTIME;
    }

    public String getDDATA_RECETIME() {
        return DDATA_RECETIME;
    }

    public void setDDATA_RECETIME(String DDATA_RECETIME) {
        this.DDATA_RECETIME = DDATA_RECETIME;
    }

    public Long getNDATA_RECDURATION() {
        return NDATA_RECDURATION;
    }

    public void setNDATA_RECDURATION(Long NDATA_RECDURATION) {
        this.NDATA_RECDURATION = NDATA_RECDURATION;
    }

    public String getSMODULATIONMODE() {
        return SMODULATIONMODE;
    }

    public void setSMODULATIONMODE(String SMODULATIONMODE) {
        this.SMODULATIONMODE = SMODULATIONMODE;
    }

    public Long getNMODULATIONRATE() {
        return NMODULATIONRATE;
    }

    public void setNMODULATIONRATE(Long NMODULATIONRATE) {
        this.NMODULATIONRATE = NMODULATIONRATE;
    }

    public String getSCODETYPE() {
        return SCODETYPE;
    }

    public void setSCODETYPE(String SCODETYPE) {
        this.SCODETYPE = SCODETYPE;
    }

    public String getSCODERATIO() {
        return SCODERATIO;
    }

    public void setSCODERATIO(String SCODERATIO) {
        this.SCODERATIO = SCODERATIO;
    }

    public String getSSCRAMBLERTYPE() {
        return SSCRAMBLERTYPE;
    }

    public void setSSCRAMBLERTYPE(String SSCRAMBLERTYPE) {
        this.SSCRAMBLERTYPE = SSCRAMBLERTYPE;
    }

    public String getSSCRAMBLERRATIO() {
        return SSCRAMBLERRATIO;
    }

    public void setSSCRAMBLERRATIO(String SSCRAMBLERRATIO) {
        this.SSCRAMBLERRATIO = SSCRAMBLERRATIO;
    }

    public String getSMODFRAMETYPE() {
        return SMODFRAMETYPE;
    }

    public void setSMODFRAMETYPE(String SMODFRAMETYPE) {
        this.SMODFRAMETYPE = SMODFRAMETYPE;
    }

    public String getSFRAMERATIO() {
        return SFRAMERATIO;
    }

    public void setSFRAMERATIO(String SFRAMERATIO) {
        this.SFRAMERATIO = SFRAMERATIO;
    }

    public Long getNINFORATE() {
        return NINFORATE;
    }

    public void setNINFORATE(Long NINFORATE) {
        this.NINFORATE = NINFORATE;
    }

    public String getSBUSINESSTYPE() {
        return SBUSINESSTYPE;
    }

    public void setSBUSINESSTYPE(String SBUSINESSTYPE) {
        this.SBUSINESSTYPE = SBUSINESSTYPE;
    }

    public String getSNETSIGNALTYPE() {
        return SNETSIGNALTYPE;
    }

    public void setSNETSIGNALTYPE(String SNETSIGNALTYPE) {
        this.SNETSIGNALTYPE = SNETSIGNALTYPE;
    }

    public Long getNDEMOD_ERR() {
        return NDEMOD_ERR;
    }

    public void setNDEMOD_ERR(Long NDEMOD_ERR) {
        this.NDEMOD_ERR = NDEMOD_ERR;
    }

    public Long getNDECOD_ERR() {
        return NDECOD_ERR;
    }

    public void setNDECOD_ERR(Long NDECOD_ERR) {
        this.NDECOD_ERR = NDECOD_ERR;
    }

    public String getSNETDATA_TYPE() {
        return SNETDATA_TYPE;
    }

    public void setSNETDATA_TYPE(String SNETDATA_TYPE) {
        this.SNETDATA_TYPE = SNETDATA_TYPE;
    }

    public String getSIS_COMPRESS_SAVE() {
        return SIS_COMPRESS_SAVE;
    }

    public void setSIS_COMPRESS_SAVE(String SIS_COMPRESS_SAVE) {
        this.SIS_COMPRESS_SAVE = SIS_COMPRESS_SAVE;
    }

    public String getSUSERCOMMENT() {
        return SUSERCOMMENT;
    }

    public void setSUSERCOMMENT(String SUSERCOMMENT) {
        this.SUSERCOMMENT = SUSERCOMMENT;
    }
}
